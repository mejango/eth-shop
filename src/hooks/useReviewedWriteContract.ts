...
export function useWriteContract(
  ...
  const writeContractAsync = useCallback(
    async (variables: Parameters<typeof mutation.writeContractAsync>[0]) => {
      // ViewAs is a revnet.money debug feature (impersonating a read-only
      // address without a wallet) that eth.shop does not carry, so its write
      // guard (`requireNoViewAs`) has no counterpart here.
      ...
      return locks
        ? locks.request(`ethshop:transaction:${keccak256(stringToHex(callKey))}`, submitReviewedCall)
        : submitReviewedCall();
    },
    [
      config,
