...
describe("Juicebox Center RPC transport", () => {
  it("does not treat other localhost ports as trusted dev clients", () => {
    ...
    expect(jbCenterAppOrigin("http://localhost:3000")).toBe("https://eth.shop");
  });
  ...
  it("routes server reads through Center with the trusted app origin", async () => {
    ...
    expect(new Headers(init.headers).get("origin")).toBe("https://eth.shop");
  });
  ...
  it.each(["https://dev.eth.shop", "http://localhost:3003"])(
